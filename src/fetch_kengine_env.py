"""
# Purpose:
Wait for the Kengine environment to be ready,
then put its data in the environment.
# Required environment variables:
- GITHUB_ENV
- GITHUB_REPOSITORY
- GITHUB_REF_NAME
- INPUT_API_TOKEN or KENGINE_API_TOKEN
- INPUT_TIMEOUT_MINUTES or KENGINE_TIMEOUT
- INPUT_APP_NAME or KENGINE_APP_NAME
"""
...
if not api_token:
    api_token = os.environ.get("KENGINE_API_TOKEN")
if not api_token:
    exit("No KENGINE_API_TOKEN provided, exiting.")

# Get the timeout
timeout_minutes = os.environ.get("INPUT_TIMEOUT_MINUTES")
if not timeout_minutes:
    timeout_minutes = os.environ.get("KENGINE_TIMEOUT")
if timeout_minutes:
    try:
        timeout_minutes = int(timeout_minutes)
    except Exception:
        exit(
            'ERROR: the KENGINE_TIMEOUT provided ("{}") is not an integer'.format(
                timeout_minutes
            )
        )
else:
...
if not app_name:
    app_name = os.environ.get("KENGINE_APP_NAME")
...
def fetch_kengine_environment():
    """Fetch the Kengine environment for this Github job"""
    ...
    if not len(response["data"]):
        exit("ERROR: no matching Kengine environment found")
...
def restart_environment(environment_id):
    """Restart the Kengine environment with the provided ID"""
...
def wait_for_environment():
    """Return the Kengine environment data once it's ready"""

    auto_restart = True
    was_restarted = False

    # Check the environment
    environment_id, environment_data = fetch_kengine_environment()
    ...
    while not environment_data["ready"]:
        ...
        if all([environment_data["retired"], auto_restart, not was_restarted]):
            ...
            was_restarted = True
            print("Restarted Kengine environment...")
        ...
        print("Waiting for Kengine environment...{}".format(wait_string))
        time.sleep(15)
...
def main():
    ...
    with open(bash_env_path, "a") as bash_env:
        bash_env.write(
            "\n".join(
                [
                    "KENGINE_BYPASS_TOKEN={}".format(environment_data["bypass_token"]),
                    "KENGINE_ENVIRONMENT_ID={}".format(environment_id),
                    "KENGINE_ENVIRONMENT_URL={}".format(environment_data["url"]),
                    "KENGINE_ENVIRONMENT_READY={}".format(environment_data["ready"]),
                    "KENGINE_ENVIRONMENT_RETIRED={}".format(
                        environment_data["retired"]
                    ),
                ]
                + ["KENGINE_ENVIRONMENT_COMMIT_HASH={}".format(commit_hash)]
                if commit_hash
                else []
            )
        )

    print("Kengine environment data written to {}!".format(bash_env_path))
